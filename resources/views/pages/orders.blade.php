...
<div class="page-header">
  <div>
    ...
    <p class="page-subtitle">Riwayat dan status semua pesanan</p>
  </div>
...
@if(session('success'))
<div class="alert alert-success mb-5 animate-fade-up" x-data x-init="setTimeout(() => $el.remove(), 4000)">
...
@php
  ...
  $isCancelled = $o->status === \App\Models\Order::STATUS_CANCELLED;
@endphp

<div class="card card-hover mb-3"
     style="{{ $isRejected ? 'border-color:var(--red-200);' : ($payStatus === 'uploaded' ? 'border-color:var(--yellow-200);' : ($isCancelled ? 'border-color:var(--gray-200);opacity:.7;' : '')) }}">
  <div class="card-p">
    ...
    <div class="flex items-start justify-between gap-3 mb-3">
      <div class="min-w-0">
        ...
        <p class="text-xs text-gray-400 mt-0.5">
          #{{ $o->id }} &middot; {{ $o->created_at->format('d M Y, H:i') }}
        </p>
      </div>
    ...
    <div class="flex flex-wrap gap-x-4 gap-y-1 mb-3">
      ...
      <p class="text-xs text-gray-500 flex items-center gap-1 min-w-0">
        <svg width="11" height="11" class="flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    <div class="alert alert-error mb-3" style="padding:8px 12px;">
      ...
      <span class="text-xs"><strong>Ditolak:</strong> {{ $o->payment_note }}</span>
    </div>
    @endif
    ...
    <div class="flex items-center justify-between gap-3">
      ...
      @if($isCancelled)
      <span class="text-xs text-gray-400 font-semibold">Dibatalkan</span>
      @elseif($isRejected)
      <a href="{{ route('order.waiting', $o->id) }}"
         class="btn btn-danger btn-sm" style="min-width:110px; justify-content:center;">
        ⚠️ Upload Ulang
      </a>
      @elseif($needUpload)
      <a href="{{ route('order.waiting', $o->id) }}"
         class="btn btn-primary btn-sm" style="min-width:110px; justify-content:center;">
      ...
      @else
      <a href="{{ route('order.waiting', $o->id) }}"
         class="btn btn-secondary btn-sm" style="min-width:80px; justify-content:center;">
        Detail
      </a>
      @endif
    </div>
